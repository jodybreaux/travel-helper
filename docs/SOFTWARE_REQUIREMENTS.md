...
- Implemented for car routes using selected departure date, departure time, selected timezone, and
  OSRM step durations.
- Trips under four hours use one midpoint recommendation stop so short local routes still surface
  nearby food and gas options.
...
- Each planned food stop includes approximate pass-through time, driving time into the trip including
  prior food stops, nearby town when available, nearby road segment, and miles from the origin.
...
- Fuel station lookup renders partial per-stop results as they arrive instead of waiting for every
  stop search to finish.
...
- The prototype uses separate HTML pages for home, route info, routes, meals, and gas, with shared
  trip state persisted in `sessionStorage` across page navigation.
- Landing-page buttons link to route info, route selection, meals, and gas pages to avoid one long
  scrollable page.
- Landing-page navigation buttons are displayed as a left-side action rail.
- Route entry lives on the dedicated `route-info.html` page.
...
- Split the single-page prototype into separate HTML pages with per-page JavaScript entry modules and
  shared trip state in `sessionStorage`.
...
- Added midpoint food and gas recommendations for short local car routes that are under two hours.
...
- Added nearby-town context to route stop timing lines.
